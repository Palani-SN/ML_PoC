...
def display_top_masks(images, mask, class_ids, class_names, limit=4):
    """Display the given image and the top few class masks."""
    z_stack = images.shape[0]
    for layer in range(z_stack):
        to_display = []
        titles = []
        image = images[layer]
        ...
        for i in range(limit):
            ...
            m = mask[layer][:, :, np.where(class_ids == class_id)[0]]
